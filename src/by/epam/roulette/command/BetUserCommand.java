...
public class BetUserCommand implements ICommand{
	...
	public PathType execute(HttpServletRequest request) {
		...
		if(user!=null){
			if((user.getMoney().signum() == 1) 
					&& ((user.getMoney().compareTo(new BigDecimal(bet)) != -1))){
				int winnum = Wheel.rotateWheel();
				request.setAttribute(WIN_NUMBER_ATTRIBUTE_NAME, winnum);
				String[]result = ServiceUser.play(user, bet, positions, winnum);
				request.setAttribute(IS_WIN_ATTRIBUTE_NAME, result[0]);
				request.setAttribute(WIN_AMOUNT_ATTRIBUTE_NAME, result[1]+"$");
				ServiceBet.addBet(user.getId(), positions, new BigDecimal(bet), result[0], new BigDecimal(result[1].trim()));
				currentPath = PATH_TO_GAME_ROOM;
			}
			else{
				request.setAttribute(IS_WIN_ATTRIBUTE_NAME, "NO MONEY");
				request.setAttribute(WIN_AMOUNT_ATTRIBUTE_NAME, "");
				currentPath = PATH_TO_GAME_ROOM;
			}
		}
		else{
